...
class Kidberries_Widget_Layered_Nav extends WP_Widget {
	...
	function Kidberries_Widget_Layered_Nav() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'kidberries_layered_nav', $this->woo_widget_name, $widget_ops );
	}
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				foreach ( $terms as $term ) {
					...
					$item .= ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'woocommerce_layered_nav_link', $link ) ) . '">' : '<span>';

					$item .= $term->name;

					$item .= ( $count > 0 || $option_is_set ) ? '</a>' : '</span>';

					//$item .= ' <small class="count">' . $count . '</small>';

					$list[] = array('count'=> $count, 'item' => $item );
					$max_count = $count > $max_count ? $count : $max_count;
				}
				
				if( ! empty($list) ) {
				    echo '<span class="widget kidberries layered-nav" style="font-size:18px;">';
					...
					echo '<h3 class="item" style="display:inline; font-size:' . (int) $size . '%;">' . $item['item'] . '</h3> ';
				    }
				    echo '</span>';
				}
