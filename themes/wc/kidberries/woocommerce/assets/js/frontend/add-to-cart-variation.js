	...
	this.find('button[type=submit].button').click( function(e){
		if( $('.variation-variants-cont select').length != $('.variation-variants-cont select.complete').length ) {
			...
			$('.variation-variants-cont select').each(function(){
				...
				$(this).animate({boxShadow: boxShadow + ", 0 0 30px " + boxShadowColor },50, function(){
					$(this).animate({boxShadow: boxShadow + ", 0 0 0px " + boxShadowColor },200);
				});
			});
		}
	});
        ...
        return this
				...
				.on( 'click', '.reset_variations', function( event ) {
					...
					var $sku 	= $("#sku");
					var $weight 	= $(this).closest('.product-esqsential').find('.product_weight');
					var $dimensions = $(this).closest('.product-esqsential').find('.product_dimensions');
