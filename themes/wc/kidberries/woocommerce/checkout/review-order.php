							...
							echo '
								<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									...
									<td class="product-name">';
										...
										printf(
											'<a href="%s">%s</a>',
											esc_url( get_permalink( apply_filters('woocommerce_in_cart_product_id', $_product->id ) ) ),
										...
										echo $_product->expected('<mark class="mark expected"> Будет отгружен %s </mark>');
									echo '</td>';
...
//										echo $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] );
    ...
    <table class="payment-methods-cont">
        <tr>
            <td class="empty border"></td>
            <th>Способ оплаты:</th>
        </tr>
